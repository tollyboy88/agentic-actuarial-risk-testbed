# Quantifying error propagation in simulated agentic actuarial workflows: a reproducible operational-risk testbed

## Abstract

Agentic artificial intelligence can turn an actuarial task into a chain of data, modelling, selection, and reporting decisions. This study presents the Agentic Actuarial Testbed (AAT), an explicitly simulated, deterministic workflow testbed with seven fault classes, eight oversight architectures, paired counterfactuals, and scenario-based loss aggregation. The screening design comprised 1,024 runs across eight synthetic insurer worlds. Among injected-fault runs, per-stage validation reduced silent failure from 81.7% in an unreviewed chain to 2.5%; the supervisor produced 4.2%. Rescaling control-detection probabilities from 60% to 140% of base values changed unresolved failure for the validator from 5.0% (95% bootstrap interval 2.5%–6.7%) to 0%. A 135-scenario capital grid showed that the lowest-VaR control was not invariant: the validator ranked first in 40.0% of scenarios, human K2 in 31.1%, and the supervisor in 28.9%. In a public CLRD2025 retrospective test across six lines of business, Chain Ladder mean absolute percentage error was 6.2%, compared with 8.9% for Bornhuetter–Ferguson and 26.3% for latest paid. Removing replay/recovery raised unresolved validator failures from 0.8% to 40.0%. The outputs are scenario-conditioned evidence from a testbed, not empirical measurements of deployed language-model agents or insurer capital.
...
**Data availability.** The study uses seeded synthetic data and the unrestricted CLRD2025 sample distributed with the open-source Chainladder project from the CAS Loss Reserve Database. Configurations, source provenance, derived results, and reproduction instructions are available at https://github.com/tollyboy88/agentic-actuarial-risk-testbed. No confidential insurer data are used.
